...
The workflow encompasses web scraping, asynchronous data processing, AI-driven relabeling, and storage in a databrick datawarehouse, with final visualization in Google Looker Studio.

Dashboard can be viewed via this: [Link](https://lookerstudio.google.com/reporting/1826b994-81a6-4250-88d1-913d0a4229b8)
...
* **Database Interface:** `sqlalchemy` for databrick warehouse interaction interactions
...
* **Robustness:** Implementation of `asyncio.semaphore` (limited to 10 concurrent requests) prevents API overloading, while exponential backoff retry logic ensures resilience against transient network failures.
...
Data is pushed to Databrick using a Star Schema design to optimize for Online Analytical Processing (OLAP).
* **Dimensional Tables:** Updated using upsert logic to handle existing records gracefully.
* **Fact Tables:** The pipeline maps newly updated dimensions to `fact_job_postings` and `fact_skill` tables using SQLAlchemy for database interactions.
